...
Commands (one per step; the key names the device):
    ...
    co2: {"percent": <%>} | <ppm> | false -> CO2 MPC target / stop (device 'relay:CO2')
...
def _parse_command(obj: dict, limits: dict):
    ...
    if cmd == 'co2':
        ...
        if isinstance(raw, dict):
            if set(raw) != {'percent'}:
                raise ProgramError('co2 object must contain only percent, e.g. {"percent": 2}')
            raw = _num(raw['percent'], 'co2 percent') * 10000
